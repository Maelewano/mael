'use client';
import Link from 'next/link';
import React from 'react';

type AlertType = 'info' | 'warning' | 'error' | 'success';

interface AlertProps {
    type?: AlertType;
    title?: string;
    message: string;
    linkText?: string;
    linkHref?: string;
    className?: string;
    onClose?: () => void;
    dismissible?: boolean;
}

const alertStyles = {
    info: {
        container: 'bg-blue-50 border-blue-400 text-blue-700',
        icon: 'text-blue-400',
        link: 'text-blue-800 hover:text-blue-900'
    },
    warning: {
        container: 'bg-yellow-50 border-yellow-400 text-yellow-700',
        icon: 'text-yellow-400',
        link: 'text-yellow-800 hover:text-yellow-900'
    },
    error: {
        container: 'bg-red-50 border-red-400 text-red-700',
        icon: 'text-red-400',
        link: 'text-red-800 hover:text-red-900'
    },
    success: {
        container: 'bg-green-50 border-green-400 text-green-700',
        icon: 'text-green-400',
        link: 'text-green-800 hover:text-green-900'
    }
};

const icons = {
    info: (
        <svg className="size-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z" clipRule="evenodd" />
        </svg>
    ),
    warning: (
        <svg className="size-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            <path fillRule="evenodd" d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" />
        </svg>
    ),
    error: (
        <svg className="size-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.28 7.22a.75.75 0 00-1.06 1.06L8.94 10l-1.72 1.72a.75.75 0 101.06 1.06L10 11.06l1.72 1.72a.75.75 0 101.06-1.06L11.06 10l1.72-1.72a.75.75 0 00-1.06-1.06L10 8.94 8.28 7.22z" clipRule="evenodd" />
        </svg>
    ),
    success: (
        <svg className="size-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.236 4.53L7.53 10.53a.75.75 0 00-1.06 1.06l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clipRule="evenodd" />
        </svg>
    )
};

export default function Alert({
    type = 'info',
    title,
    message,
    linkText,
    linkHref,
    className = '',
    onClose,
    dismissible = false
}: AlertProps) {
    const styles = alertStyles[type];

    const handleClose = () => {
        if (onClose) {
            onClose();
        } else {
            // Default behavior: hide the alert element
            const alertElement = document.querySelector(`[data-alert-id="${Math.random()}"]`);
            if (alertElement) {
                alertElement.remove();
            }
        }
    };

    return (
        <div className={`rounded-md border-l-4 p-4 ${styles.container} ${className}`}>
            <div className="flex">
                <div className="shrink-0">
                    <div className={styles.icon}>
                        {icons[type]}
                    </div>
                </div>
                <div className="ml-3 flex-1">
                    <div className="text-sm">
                        {title && (
                            <strong className="font-medium">{title}:</strong>
                        )}
                        <span className={title ? ' ml-1' : ''}>
                            {message}
                            {linkText && linkHref && (
                                <>
                                    {' '}
                                    <Link
                                        href={linkHref}
                                        className={`font-medium underline ${styles.link}`}
                                    >
                                        {linkText}
                                    </Link>
                                </>
                            )}
                        </span>
                    </div>
                </div>
                {dismissible && (
                    <div className="ml-auto pl-3">
                        <div className="-m-1.5">
                            <button
                                type="button"
                                onClick={handleClose}
                                className={`inline-flex rounded-md p-1.5 focus:outline-none focus:ring-2 focus:ring-offset-2 ${styles.icon} hover:bg-black/20`}
                            >
                                <span className="sr-only">Dismiss</span>
                                <svg className="size-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>
                            </button>
                        </div>
                    </div>
                )}
            </div>
        </div>
    );
}