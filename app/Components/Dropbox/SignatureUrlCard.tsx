...
export default function SignatureUrlCard() {
      ...
      if (urlSignatureId) {
        ...
        const fetchSignUrl = async () => {
          ...
          try {
            ...
            if (!res.ok) {
              // If server indicates a locked/waiting state, show that message directly
              ...
              throw new Error(
                `Embed Sign URL: ${errorMessage} (Status: ${res.status})`,
              );
            }
  ...
  // Initialize HelloSign client only on client side and clean up on unmount
  useEffect(() => {
    let mounted = true;
  ...
  useEffect(() => {
    ...
    if (
      ...
      try {
        ...
        if (
          ...
          env.NEXT_PUBLIC_DROPBOX_SIGN_CLIENT_ID
        ) {
        ...
        // Close any existing session to avoid HelloSign trying to postMessage to a removed iframe
        if (typeof helloSignClient.close === "function") {
          try {
        ...
        // Wait for the DOM to settle before opening the embedded signer
        rafId = requestAnimationFrame(() => {
          ...
          try {
            ...
            setError(
              `Failed to initialize the signing experience. (${(err as Error).message})`,
            );
          }
        });
      } catch (err) {
        console.error("Error preparing HelloSign:", err);
        setError(
          `Failed to initialize the signing experience. (${(err as Error).message})`,
        );
      }
    ...
    return () => {
      ...
      if (helloSignClient && typeof helloSignClient.close === "function") {
        try {
          helloSignClient.close();
        } catch (e) {
          /* ignore */
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (email) {
        const res = await fetch(
          `/api/esignature?email=${encodeURIComponent(email)}`,
        );
        const data = await res.json();
        if (!res.ok) {
          throw new Error(
            data.error || "Could not find signature for this email",
          );
        }
        // Find the first signature for this email
        const foundSig = (data.signatures || []).find(
          (sig: any) => sig.signerEmailAddress === email,
        );
        if (!foundSig) {
      ...
      if (!res2.ok) {
        if (res2.status === 409 && data2?.error) {
          setError(data2.error);
          return;
        }

        const errorMessage =
          data2.error?.errorMsg ||
          data2.error ||
          "Error retrieving embedded sign URL";
        throw new Error(
          `Embed Sign URL: ${errorMessage} (Status: ${res2.status})`,
        );
      }
      setResponse(data2);
    } catch (err) {
      ...
      {response?.success && response.embedded && (
        <div className="mt-6 size-full space-y-4">
          ...
          {process.env.NODE_ENV === "development" && (
            <div className="mx-6 my-6 rounded-xl border border-gray-200 p-2 bg-white flex text-center">
          ...
          <div className="mx-6 mb-6">
            <div className="mb-4 flex flex-row items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="rounded-md bg-gray-100 px-2 py-1 text-sm text-gray-600">
                  {new Date(
                    response.embedded.expires_at * 1000,
                  ).toLocaleString()}
                </span>
              </div>
            </div>
            ...
            <div
              ...
              className="h-[800px] w-full overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm"
            />
          </div>
        </div>
      )}
